...
RFM69.prototype.reset = function(callback) {
	...
	var scope = this;
	this.resetPin.write(1, function(err) {
		setTimeout(function() {
			scope.resetPin.write(0, function(err) {
...
RFM69.prototype.setInterrupt = function(enable) {
	var scope = this;
	if (enable) {
		this.interruptPin.watch(function(err, value) {
			if (err) {
				throw err;
			}
...
RFM69.prototype.configure = function(callback) {
	...
	for (var key in registers) {
		...
		this.setRegister(parseInt(key), registers[key], function(e, d) {
			i++;
			if (e) {
...
RFM69.prototype.loadRegistersFromModule = function(callback) {
	...
	for (var key in registers) {
		...
		this.loadRegisterValue(parseInt(key), function() {
			i++;
			if (i >= registerCount) {
				callback();
			}
...
RFM69.prototype.verifySync = function(callback) {
	var scope = this;
	scope.setRegister(REG_SYNC_VALUE_1, 0xAA, function(err, data) {
		scope.readRegister(REG_SYNC_VALUE_1, function(err, data) {
			if (data[1] === 0xAA) {
				scope.setRegister(REG_SYNC_VALUE_1, 0x55, function(err, data) {
					scope.readRegister(REG_SYNC_VALUE_1, function(err, data) {
						if (data[1] === 0x55) {
							callback(true);
						}
...
RFM69.prototype.changeMode = function(desiredMode, callback) {
	var scope = this;
	scope.detectMode(function(currentMode) {
		var newMode = (currentMode & parseInt('11100011', 2)) | desiredMode;
		scope.setRegister(REG_OP_MODE, newMode, function(err, data) {
			if (err) {
...
RFM69.prototype.standbyMode = function(callback) {
	var scope = this;
	this.changeMode(parseInt('00000100', 2), function(err, data) {
		if (err) {
...
RFM69.prototype.transmitMode = function(callback) {
	var scope = this;
	this.changeMode(parseInt('00001100', 2), function(err, data) {
		if (err) {
			...
			scope.powerLevel(true, function(err, data) {
				if (err) {
...
RFM69.prototype.receiveMode = function(callback) {
	...
	this.changeMode(parseInt('00010000', 2), function(err, data) {
		if (err) {
			...
			scope.powerLevel(false, function(err, data) {
				if (err) {
...
RFM69.prototype.detectMode = function(callback) {
	var scope = this;
	scope.readRegister(REG_OP_MODE, function(e, d) {
		callback(d[1]);
	});
};
...
RFM69.prototype.doAfterReady = function(todo) {
	var scope = this;
	setTimeout(function() {
		scope.readRegister(REG_IRQ_FLAGS_1, function(err, data) {
			if (err) {
...
RFM69.prototype.powerLevel = function(high, callback) {
	...
	var scope = this;
	if (high) {
		pa1 = parseInt('01011101', 2);
		pa2 = parseInt('01111100', 2);
	} else {
		pa1 = parseInt('01010101', 2);
		pa2 = parseInt('01110000', 2);
	}
	this.setRegister(REG_TEST_PA_1, pa1, function(err, data) {
		if (err) {
			console.error('Unable to set first power level register', err);
			callback(err, data);
		} else {
			scope.setRegister(REG_TEST_PA_2, pa2, function(err, data) {
				if (err) {
...
RFM69.prototype.enableReception = function(callback) {
	var scope = this;
	this.readRegister(REG_IRQ_FLAGS_2, function(err, data) {
		if (data[1] & 0x04) {
			console.log('payload ready');
		}
		scope.setRegister(REG_DIO_MAPPING_1, 0x40, function(err, data) {
...
RFM69.prototype.interrupted = function(value) {
	...
	this.readRegister(REG_IRQ_FLAGS_2, function(err, data) {
		console.log('read register', err, data);
		if (data[1] & 0x08) {
			console.log('Packet Sent');
			if (scope.interruptListeners[0x08]) {
				scope.interruptListeners[0x08].forEach(function(listener) {
					listener();
				});
			...
			scope.standbyMode(function(err, data) {
				...
				scope._transfer(headerBuffer, function(err, headerData) {
					...
					scope._transfer(payloadBuffer, function(err, payloadData) {
						var modifiedPayload = payloadData.slice(1);

						if (scope.verbose) {
							console.log('payload received by rfm69', modifiedPayload);
						}
...
RFM69.prototype.send = function(buffer, callback) {
	...
	scope.detectMode(function(originalMode) {
		...
		scope._attemptSend(buffer, function(err, data) {
			...
			scope.changeMode(originalMode, function(err, data) {
				if (err) {
	...
	scope.standbyMode(function(err, data) {
		if (err) {
			...
			scope.doAfterReady(function() {
				scope.setRegister(REG_DIO_MAPPING_1, 0x00, function(err, data) {
					if (err) {
						...
						scope._transfer(payload, function(error, data) {
							if (error) {
								console.error('Send attempt failed while transferring', error);
								callback(error, data);
							} else {
								scope.doAfterInterrupt(0x08, function() {
									if (scope.verbose) {
										console.log('send attempt post interrupt');
									}
									scope.receiveMode(function(err, data) {
										if (err) {
											console.error('Railed while switching to receive mode after transmit', err);
										} else {
											if (scope.verbose) {
								...
								scope.doAfterInterrupt(0x04, function() {
									var done = true;
									if (typeof scope.config.payloadSent === 'function') {
										if (scope.verbose) {
											console.log('custom payload handler');
										}
										done = scope.config.payloadSent(buffer, callback);
									}

									if (done) {
										if (scope.verbose) {
											console.log('standard payload handler');
										}
										scope.standbyMode(function(err, data) {
											if (err) {
												console.error('Send attempted failed to switch back to standby mode', err);
											}
											callback(err, data);
										});
									}
								});
								scope.transmitMode(function(err, data) {
									if (err) {
...
RFM69.prototype.readRegister = function(address, callback) {
	...
	this._transfer(buffer, function(e, d) {
		if (scope.verbose) {
			console.log('register read', address.toString(16), '=>', d[1].toString(2));
		}
		if (typeof callback !== 'function') {
			console.log('not a callback', address, callback);
		}
		callback(e, d);
	});
};
RFM69.prototype.setRegister = function(address, value, callback) {
	...
	this._transfer(buffer, function(e, d) {
		...
		if (typeof callback !== 'function') {
			console.log('not a callback', address.toString(16), value, callback);
		}
		callback(e, d);
	});
};
