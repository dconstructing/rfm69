...
RFM69.prototype.changeMode = function(desiredMode, callback) {
	...
	scope.detectMode(function(currentMode) {
		...
		scope.setRegister(REG_OP_MODE, newMode, function(e, d) {
			callback();
		});
	});
};
RFM69.prototype.standbyMode = function(callback) {
	this.changeMode(parseInt('00000100', 2), callback);
};
RFM69.prototype.transmitMode = function(callback) {
	var scope = this;
	this.changeMode(parseInt('00001100', 2), function() {
		scope.powerLevel(true, callback);
	});
};
RFM69.prototype.receiveMode = function(callback) {
	...
	this.changeMode(parseInt('00010000', 2), function() {
		scope.powerLevel(false, callback);
	});
};
...
RFM69.prototype.doAfterReady = function(todo) {
	var scope = this;
	setTimeout(function() {
		scope.readRegister(REG_IRQ_FLAGS_1, function(e, d) {
			if ((d[1] & parseInt('10000000', 2)) === parseInt('10000000', 2)) {
				//ready
				todo();
			} else {
				// not ready yet
...
RFM69.prototype.doAfterInterrupt = function(todo) {
	this.interruptListeners.push(todo);
};
RFM69.prototype.powerLevel = function(high, callback) {
	...
	this.setRegister(REG_TEST_PA_1, pa1, function() {
		scope.setRegister(REG_TEST_PA_2, pa2, function() {
			callback();
		});
	});
};
...
RFM69.prototype.interrupted = function() {
	var scope = this;
	console.log('interrupted');
	scope.interruptListeners.forEach(function(listener) {
		listener();
	});
	scope.interruptListeners = [];
	this.readRegister(REG_IRQ_FLAGS_2, function(err, data) {
		if (data[1] & 0x04) {
			scope.standbyMode(function() {
				...
				scope._transfer(headerBuffer, function(err, headerData) {
					...
					scope._transfer(payloadBuffer, function(err, payloadData) {
						if (scope.verbose) {
							console.log('payload received by rfm69', payloadData.slice(1));
						}
						if (payloadData[3] & 0x80) {
							//ack
							if (typeof scope.ackCallback === 'function') {
								scope.ackCallback();
							}
						}
						scope.config.handlePayload(payloadData.slice(1), scope.receiveHandler);
						scope.receiveMode(function() {});
					});
				});
			});
...
RFM69.prototype.send = function(buffer, callback) {
	...
	scope.detectMode(function(originalMode) {
		...
		scope._attemptSend(buffer, function(err) {
			scope.changeMode(originalMode, function() {
				if (scope.verbose) {
		...
		scope.doAfterReady(function() {
			scope.setRegister(REG_DIO_MAPPING_1, 0x00, function(err, data) {
				...
				scope._transfer(payload, function(error, data) {
					scope.doAfterInterrupt(function() {
						var done = true;
						if (typeof scope.config.payloadSend === 'function') {
