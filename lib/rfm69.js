...
RFM69.prototype.doAfterReady = function(todo) {
	var scope = this;
	setTimeout(function() {
		scope.readRegister(REG_IRQ_FLAGS_1, function(err, data) { // 0x27
			if (err) {
...
RFM69.prototype.powerLevel = function(high, callback) {
	...
	this.setRegister(REG_TEST_PA_1, pa1, function(err, data) { // 0x5A
		if (err) {
			console.error('Unable to set first power level register', err);
			callback(err, data);
		} else {
			scope.setRegister(REG_TEST_PA_2, pa2, function(err, data) { // 0x5C
				if (err) {
...
RFM69.prototype.enableReception = function(callback) {
	var scope = this;
	this.readRegister(REG_IRQ_FLAGS_2, function(err, data) { // 0x28
		if (data[1] & 0x04) {
			console.log('payload ready');
		}
		scope.setRegister(REG_DIO_MAPPING_1, 0x40, function(err, data) { // 0x25
