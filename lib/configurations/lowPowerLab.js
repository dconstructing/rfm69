...
var lplConfig = {
	...
	payloadSent: function(buffer, callback) {
		console.log('using low power lab payload handler');
		var timeout;
		...
		scope.rfm69.receiveMode(function(err, data) {
			if (err) {
				console.error('lowPowerLab failed to switch to receive mode', err);
			}
		});
		return false;
	},
